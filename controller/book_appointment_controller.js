...
async function getsinglesappointment(req, res){
  try {
    ...
    const result = await db.one('SELECT * FROM book_appointment WHERE id='+req.params.id)
    return res.status(200).json(result);
    console.log(result)
  } catch (error) {
    return res
...
async function deleteappointment(req, res){
  try {
    const result = await db.none("delete from book_appointment where id="+req.body.id);
